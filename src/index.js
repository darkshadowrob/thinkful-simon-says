...
const skillLevelContainer = document.querySelector('.js-skill-level');
...
function startButtonHandler() {
  ...
  setLevel();
  roundCount++;
  skillLevelContainer.classList.add('hidden');  // Hide the container
...
// The level parameter determines the game's difficulty level. 
// The maximum round count is controlled by the level chosen. 
function setLevel(level) {
  level ??= Number(document.querySelector('select[name="skill-level"]').value);
  const rounds = [8, 14, 20, 31];
  maxRoundCount = rounds[level - 1];
  if (level < 1 || level > 4) {
...
function activatePad(color) {
  // TODO: Write your code here.
  // Find the pad object with the matching color
  const pad = pads.find(pad => pad.color === color);

  // Check if a valid pad object is found
  if (pad) {
    // Add activated class to the pad's selector
    pad.selector.classList.add("activated");

    // After 500 milliseconds, set a timeout to eliminate the "activated" class.
    setTimeout(() => pad.selector.classList.remove("activated"), 500);

    // Reset the sound's current time so that it starts over
...
function playComputerTurn() {
  // TODO: Write your code here.
  // Disable the user interaction with the padContainer
  padContainer.classList.add('unclickable');

  // Update statusSpan to reflect that it is the computer's turn
  statusSpan.textContent = "The computer's turn...";

  // Update the heading to display the current and max round count
  heading.textContent = `Round ${roundCount} of ${maxRoundCount}`;

  // Push random color to computerSequence array
  computerSequence.push(getRandomItem(pads).color);

  // Activate the pads based on computerSequence
...
function playHumanTurn() {
  // TODO: Write your code here.
  // To make each pad clickable once more, remove unclickable class from the pad container.
  padContainer.classList.remove("unclickable");

  // Determine the number of presses that are left in the round.
  const remainingPresses = computerSequence.length - playerSequence.length;


  statusSpan.textContent = `${remainingPresses} presses remaining`;
}
...
function checkPress(color) {
  ...
  // Update the player's sequence with the current color.
  playerSequence.push(color);

  // Find the current color's index in the player's sequence.
  const index = playerSequence.length - 1;

  // Calculate how many presses are left in the round.
  const remainingPresses = computerSequence.length - playerSequence.length;

  // Record on the console how many more presses there are.
  setText(statusSpan, `${remainingPresses} presses remaining`);

  // Determine whether the player's color corresponds to the equivalent color in the computer's sequence.
  if (color !== computerSequence[index]) {
    // The turn is incorrect if the colors don't match
    resetGame("Game Over!");
    return;
  }
  ...
  if (playerSequence.length === computerSequence.length) {
    // Check the round if the player has completed the sequence.
    checkRound();
  }
}
...
function checkRound() {
  // TODO: Write your code here.
  // Determine whether the length of the playerSequence is equal to the maxRoundCount. If true, the game is reset and a "Congratulations!" message will be shown, otherwise, then the next round with "Good Job!"
  if (playerSequence.length === maxRoundCount) {
    resetGame("Congratulations!");
  } else {
    roundCount++;
    playerSequence = [];
    setText(statusSpan, "Good Job!");
    setTimeout(playComputerTurn, 1000); // 1 second
  }
}
...
function resetGame(text) {
  // TODO: Write your code here.
  //Empty list to store computer and player's seuqence. Round count to 0.
  computerSequence = [];
  playerSequence = [];
  roundCount = 0;
